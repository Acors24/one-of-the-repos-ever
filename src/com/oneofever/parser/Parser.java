...
public class Parser {
    ...
    public AbstractCommand parse(String[] toParse) throws ParseException {
        ...
        Boolean end =
                false; // is command finished (we might remove this in the future if we will need
        // more that one command in line)
        ...
        for (String a : toParse) {
            ...
            if (activeCommand == null) {
                Optional<AbstractCommand> match =
                        commands.stream().filter(σ -> σ.name().equals(a)).findFirst();
                if (match.isPresent()) {
            ...
            if (activeGroupIndex < 0) {
                ...
                if (activeGroupIndex >= 0) {
                    ...
                    if (looseCnt != 0) {
                        // type parse
                        if (isArgumentOfType(
                                a, activeCommand.argGroups.get(activeGroupIndex).argType)) {
        ...
        // System.out.println("agidx:"+activeGroupIndex+" groups:"+groups+" loosecnt:"+looseCnt+"
        // groupCnt:"+groupCnt);
        // activeCommand.print();
